 ...
 */
/**
 * Indicatori sugerați de analiza AI Live (SL/TP). Validează și limitează numărul de serii.
 * Acceptă câmpuri `tip` sau `indicator` (EMA, SMA, BB / BOLLINGER).
 * @param {unknown} items
 */
export function overlaySpecsFromLiveAiIndicators(items) {
  ...
  for (const raw of items) {
    if (!raw || typeof raw !== "object" || specs.length >= MAX) break;
    const tip = String(raw.tip ?? raw.indicator ?? raw.type ?? "")
      .trim()
      .toUpperCase();

    if (tip === "EMA") {
      const period = Math.max(2, Math.min(200, Math.floor(Number(raw.period ?? 20))));
      const key = `ema:${period}`;
      if (seen.has(key)) continue;
      seen.add(key);
      specs.push({
      ...
      const period = Math.max(2, Math.min(200, Math.floor(Number(raw.period ?? 50))));
      const key = `sma:${period}`;
      if (seen.has(key)) continue;
      seen.add(key);
      specs.push({
    ...
    } else if (tip === "BB" || tip === "BOLLINGER") {
      const period = Math.max(2, Math.min(200, Math.floor(Number(raw.period ?? 20))));
      const mult = Math.max(1, Math.min(4, Number.isFinite(Number(raw.mult)) ? Number(raw.mult) : 2));
      const key = `bb:${period}:${mult}`;
      if (seen.has(key)) continue;
      seen.add(key);
      specs.push({
...
/** Cheie stabilă pentru deduplicare între overlay-uri strategie și AI. */
export function overlaySpecKey(spec) {
  if (!spec || typeof spec !== "object") return "";
  if (spec.key != null && String(spec.key).length) return String(spec.key);
  if (spec.kind === "bb") return `bb:${spec.period}:${spec.mult}`;
  return `${spec.kind}:${spec.period}`;
}

/**
 * Combină specificațiile din strategie cu cele din analiza AI (fără duplicate).
 * @param {ReturnType<typeof buildStrategyOverlaySpecs>} strategySpecs
 * @param {ReturnType<typeof overlaySpecsFromLiveAiIndicators>} aiSpecs
 */
export function mergeStrategyAndAiOverlaySpecs(strategySpecs, aiSpecs) {
  const a = Array.isArray(strategySpecs) ? strategySpecs : [];
  const b = Array.isArray(aiSpecs) ? aiSpecs : [];
  const seen = new Set();
  const out = [];
  for (const s of [...a, ...b]) {
    const k = overlaySpecKey(s);
