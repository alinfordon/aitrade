...
/**
 * Elimină (fără vânzare) o poziție din `manualSpotBook` — util pentru „praf”
 * care nu respectă LOT_SIZE / MIN_NOTIONAL pe Binance și nu mai poate fi vândut.
 * `force` permite ștergerea și când serverul nu confirmă că e praf.
 */
export const liveDiscardSchema = z.object({
  pair: normPairString,
...
export const aiPilotSettingsSchema = z.object({
  ...
  /**
   * Guard anti-intrări târzii pe 15m (ex. după vârf + scădere bruscă).
   * Dacă lipsește din DB, tratăm ca enabled.
   */
  momentumGuardEnabled: z.boolean().optional(),
  momentumGuardStrictness: z.enum(["permissive", "balanced", "strict"]).optional(),
  /**
   * Custom thresholds (override) pentru guard-ul momentum.
   * Procentele sunt în procente (ex. 0.05 = 0.05%).
   */
  momentumGuardCustomEnabled: z.boolean().optional(),
  momentumGuardMinLastChangePct: z.number().min(0).max(1).optional(), // ex. 0.05%
  momentumGuardMinAccelerationPct: z.number().min(-1).max(0).optional(), // ex. -0.05%
  momentumGuardMaxDrawdownFromHighPct: z.number().min(0.1).max(20).optional(), // ex. 2.5%
...
export const manualLiveTpslSettingsSchema = z.object({
  enabled: z.boolean().optional(),
  intervalMinutes: z.number().int().min(1).max(30).optional(),
});
...
const symbolString = z
  ...
  .min(1)
  .max(24)
  .transform((s) => String(s).trim().toUpperCase().replace(/[^A-Z0-9]/g, ""))
  .refine((s) => s.length >= 1, { message: "Simbol invalid" });
...
export const portfolioManualHoldingSchema = z.object({
  symbol: symbolString,
...
export const portfolioUpdateSchema = z.object({
  tolerancePct: z.number().min(0.5).max(30).optional(),
  dustThresholdUsd: z.number().min(0).max(1000).optional(),
  includeRealSpot: z.boolean().optional(),
  includeManual: z.boolean().optional(),
  targets: z.array(portfolioTargetSchema).max(50).optional(),
  manualHoldings: z.array(portfolioManualHoldingSchema).max(100).optional(),
});
