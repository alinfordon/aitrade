...
/**
 * Piața Binance nu acceptă ordin (întreținere, simbol oprit etc.) — nu are sens să persistăm Trade `failed`.
 * Acceptă `Error`, string sau obiect CCXT cu `message` / `msg`.
 * @param {unknown} err
 */
export function isBinanceMarketClosedError(err) {
  let raw = "";
  let code = null;
  if (err instanceof Error) {
    ...
    code = o.code;
    raw = String(o.msg ?? o.message ?? "");
  } else {
    raw = String(err ?? "");
  }
  const combined = `${code != null ? `${code} ` : ""}${raw}`;
  if (/market is closed/i.test(combined)) return true;
  if (/NO_TRADING_WINDOW|no trading window could be found/i.test(combined)) return true;
  if (
    /symbol is not trading|not in trading status|halt trading|trading halt|suspend.+trading|trading suspended|exchange maintenance/i.test(
      combined
    )
  ) {
    return true;
  }
  if (/order book is closed|exchange is closed|market .+ unavailable for trading/i.test(combined)) return true;
  return false;
}
...
export function mapBinanceUserMessage(err) {
  ...
  if (/NOTIONAL|-1013|Filter failure:\s*NOTIONAL/i.test(raw)) {
    return `${raw.trim()} — Binance cere pentru ordinul Spot o valoare minimă în moneda perechii (filtru NOTIONAL). Sub acel prag, ordinul Market nu poate fi plasat nici din API. În app-ul Binance, „Convert” folosește alt flux și uneori acceptă sume mai mici — nu înseamnă că Spot Market sub prag ar fi valid. Micșorează poziția în carte dacă e praf sau folosește Convert pe Binance.`;
  }
  ...
  if (/market is closed|NO_TRADING_WINDOW|not in trading status|trading halt/i.test(raw)) {
    return `${raw.trim()} — Piața pentru acest simbol e oprită sau în mentenanță pe Binance; ordinul nu s-a trimis. Reîncearcă când tranzacționarea e din nou activă.`;
  }
