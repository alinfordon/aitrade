...
const publicExchangeCache = {
  spot: { ex: null, marketsLoadedAt: 0 },
  futures: { ex: null, marketsLoadedAt: 0 },
};

const priceCache = new Map();

function exchangeCacheKey(futures) {
  return futures ? "futures" : "spot";
}

function priceCacheKey(symbol, futures) {
  return `${futures ? "fut" : "spot"}:${String(symbol).trim().replace(/-/g, "/")}`;
}

async function getPublicExchange(opts = {}) {
  const key = exchangeCacheKey(Boolean(opts.futures));
  const slot = publicExchangeCache[key];
  if (!slot.ex) {
    slot.ex = createExchange({ futures: Boolean(opts.futures) });
    slot.marketsLoadedAt = 0;
  }
  const now = Date.now();
  if (!slot.marketsLoadedAt || now - slot.marketsLoadedAt > MARKETS_CACHE_MS) {
    await withRetries(() => slot.ex.loadMarkets());
    slot.marketsLoadedAt = now;
  }
  return slot.ex;
}
...
/**
 * Cât BASE poate fi vândut efectiv pe spot după LOT_SIZE, min qty și min notional.
 * 0 = totul e „praf” la prețul dat (botul poate închide poziția în app și trece la buy).
 */
export function spotMaxSellableBaseFromFree(freeBase, market, price) {
  const m = market;
  const raw = Number(freeBase);
  if (!m || !(Number.isFinite(raw) && raw > 0)) return 0;
  const lot = readLotSizeFilter(m);
  let qty = raw;
  if (lot.stepSize != null) qty = floorQtyToLotStep(qty, lot.stepSize);
  if (!(qty > 0)) return 0;
  const minAmt = lot.minQty ?? m.limits?.amount?.min ?? null;
  if (minAmt != null && qty + 1e-12 < Number(minAmt)) return 0;
  const px = Number(price);
  if (Number.isFinite(px) && px > 0) {
    const minNotional = minSpotNotionalFromMarket(m);
    if (minNotional != null && qty * px + 1e-12 < minNotional) return 0;
  }
  return qty;
}

/** Exchange + piață CCXT pentru o pereche spot (pentru sincronizare sold / praf). */
export async function fetchBinanceSpotMarket(apiKey, secret, pair) {
  const ex = createExchange({ apiKey, secret, futures: false });
  await syncServerTime(ex);
  await withRetries(() => ex.loadMarkets());
  await syncServerTime(ex);
  const symNorm = String(pair || "").trim().replace(/-/g, "/");
  const sym = resolveBinanceSpotSymbol(ex, symNorm);
  if (!sym) return null;
  return { exchange: ex, symbol: sym, market: ex.markets[sym] };
}
...
export async function getPrice(symbol, opts = {}) {
  const ck = priceCacheKey(symbol, Boolean(opts.futures));
  const cached = priceCache.get(ck);
  const now = Date.now();
  if (cached && now - cached.ts < PRICE_CACHE_MS) {
    return cached.value;
  }

  const ex = await getPublicExchange({ futures: opts.futures });
  ...
  const value = Number(t.last || t.info?.lastPrice || 0);
  if (Number.isFinite(value) && value > 0) {
    priceCache.set(ck, { value, ts: now });
  }
  return value;
}

/**
 * Prețuri spot batch pentru simboluri (ex. BTC/USDC).
 * Returnează Map<symbolNormalizat, price>.
 * Folosește cache local și un singur fetchTickers pentru simbolurile lipsă.
 * @param {string[]} symbols
 * @param {{ futures?: boolean }} [opts]
 */
export async function getPricesBatch(symbols, opts = {}) {
  const futures = Boolean(opts.futures);
  const out = new Map();
  const now = Date.now();
  const uniqInput = [...new Set((Array.isArray(symbols) ? symbols : []).map((s) => String(s).trim().replace(/-/g, "/")))]
    .filter(Boolean);
  if (uniqInput.length === 0) return out;

  const ex = await getPublicExchange({ futures });
  const missing = [];

  for (const symbol of uniqInput) {
    let resolved = symbol;
    if (!futures) {
      resolved = resolveBinanceSpotSymbol(ex, symbol);
      if (!resolved) continue;
    }
    const ck = priceCacheKey(resolved, futures);
    const cached = priceCache.get(ck);
    if (cached && now - cached.ts < PRICE_CACHE_MS) {
      out.set(symbol, cached.value);
      continue;
    }
    missing.push({ requested: symbol, resolved });
  }

  if (missing.length > 0) {
    try {
      const tickers = await withRetries(() => ex.fetchTickers(missing.map((m) => m.resolved)));
      for (const item of missing) {
        const t = tickers?.[item.resolved];
        const value = Number(t?.last || t?.info?.lastPrice || 0);
        if (Number.isFinite(value) && value > 0) {
          priceCache.set(priceCacheKey(item.resolved, futures), { value, ts: now });
          out.set(item.requested, value);
        }
      }
    } catch {
      // Fallback: dacă fetchTickers nu răspunde pentru set, încearcă individual.
      await Promise.all(
        missing.map(async (item) => {
          try {
            const value = await getPrice(item.resolved, { futures });
            if (Number.isFinite(value) && value > 0) out.set(item.requested, value);
          } catch {
...
export async function placeMarketSellSpotClamped({ apiKey, secret, symbol, amountBase }) {
  ...
  if (minNotional != null && estNotional + 1e-12 < minNotional) {
    const err = new Error(
      `Spot MARKET pe ${sym}: valoarea ordinului (~${estNotional.toFixed(6)} ${quote}) e sub minimul oficial Binance pentru această pereche (${minNotional} ${quote}, NOTIONAL / aplicabil și pieței). ` +
        `API-ul Spot nu trimite ordine sub acest prag — Binance le respinge la fel. ` +
        `Dacă în Binance „merge”: probabil nu e ordin Spot clasic, ci Convert sau alt produs (reguli diferite). Pentru rest sub prag folosește Convert în app-ul Binance sau acumulează peste ${minNotional} ${quote}.`
    );
