...
export async function runAutopilotDecide(ctx) {
  const { gainersSlice, botsPayload, manualPayload, limite, perechiBotiExistente, aiRuntime } = ctx;
  const payload = {
...
7b. BUGET: suma tuturor „sumaUsdc” propuse în „cumpara” NU trebuie să depășească limite.sumaQuoteDisponibila (USDC lichid disponibil). Dacă limite.sumaQuoteDisponibila este < 2, NU propune cumpărări (returnează „manual”: [] sau doar vânzări). Fiecare „sumaUsdc” <= limite.maxUsdcPerOrdinBuy.
...
${BOTNOU_HINT}`;
  ...
  let raw;
  try {
    raw = await generateAiText(prompt, { jsonMode: true, temperature: 0.4, maxOutputTokens: 8192 }, aiRuntime);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (!/400|unsupported|responseMimeType|json|Ollama API/i.test(msg)) {
      throw e;
    }
    raw = await generateAiText(prompt, { jsonMode: false, temperature: 0.4, maxOutputTokens: 8192 }, aiRuntime);
  }
