...
export async function runMarketAiAnalysis(ctx) {
  const { gainers, trending, aiRuntime } = ctx;
...
${OUTPUT_SCHEMA}`;
  ...
  let raw;
  try {
    raw = await generateAiText(prompt, { jsonMode: true, temperature: 0.55, maxOutputTokens: 8192 }, aiRuntime);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (!/400|unsupported|responseMimeType|json|Ollama API/i.test(msg)) {
      throw e;
    }
    raw = await generateAiText(prompt, { jsonMode: false, temperature: 0.55, maxOutputTokens: 8192 }, aiRuntime);
  }
