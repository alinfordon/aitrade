 ...
 * @param {{ baseUrl?: string, model?: string, apiKey?: string }} creds
...
export async function ollamaGenerateText(prompt, opts = {}, creds = {}) {
  ...
  if (!r.ok) {
    ...
    if (r.status === 401) {
      throw new Error(
        "Ollama: 401 — lipsește sau e invalidă autentificarea. " +
          "Pentru https://ollama.com/api sau un gateway/proxy cu Bearer, setează cheia în Setări (Ollama API key) " +
          "sau variabila OLLAMA_API_KEY pe server; cererea trimite «Authorization: Bearer …»."
      );
    }
