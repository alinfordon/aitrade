...
export function clampProtectForLongSpot(avgEntry, markPrice, sl, tp) {
  ...
  /**
   * Buffer minim de siguranță între intrare și SL / TP. Un SL la 0.1% sub
   * intrare este matematic imposibil de păstrat din cauza fee-ului taker
   * (~0.1% × 2) + spread + micro-volatilitate; poziția se închide pe zgomot
   * la câteva secunde. Forțăm cel puțin 1% distanță pe ambele părți.
   */
  s = Math.max(floorAbs, Math.min(s, e * 0.99));
  t = Math.min(capTp, Math.max(t, e * 1.01));

  if (s >= e * 0.99) s = e * 0.99;
  if (t <= e * 1.01) t = e * 1.01;
