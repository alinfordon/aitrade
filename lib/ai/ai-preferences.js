...
export function resolveAiCredentials(user) {
  ...
  return { geminiKey, geminiModel, anthropicKey, anthropicModel, ollamaBaseUrl, ollamaModel };
}
...
export function missingProviderKeyMessage(provider) {
  ...
  if (provider === "ollama") {
    return "Ollama: setează URL de bază și model în Setări sau OLLAMA_BASE_URL + OLLAMA_MODEL pe server (modelul trebuie tras local: ollama pull …).";
  }
